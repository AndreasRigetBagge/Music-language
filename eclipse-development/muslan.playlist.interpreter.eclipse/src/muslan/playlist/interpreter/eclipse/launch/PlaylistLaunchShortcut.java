package muslan.playlist.interpreter.eclipse.launch;
...
public class PlaylistLaunchShortcut implements ILaunchShortcut {

	@Override
	public void launch(ISelection selection, String mode) {
		IFile file = getFirstIFileFromSelectionWithExtension("playlist", selection);
		Playlist playlist = (Playlist) load(file);
		IContainer container = file.getParent();
		execute(playlist, container);
	}
	...
	public void launch(IEditorPart editor, String mode) {
		...
		if (model instanceof Playlist playlist && container != null) {
			execute(playlist, container);
			return;
		}
	...
	protected void execute(Playlist playlist, IContainer container){ 
		IFile file = container.getFile(new Path("answer.queue"));

		try {
			save(PlaylistInterpreter.interpret(playlist), file);
