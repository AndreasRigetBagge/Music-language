...
/*
	public String interpret(StateMachine stateMachine, String interpreterInput, OutputStream interpreterOutputStream) throws QueryInterpreterException {
		...
		if (currentState == null) {
			throw new QueryInterpreterException("State machine \"" + stateMachine.getName() + "\" does not specify an initial state.");
		}
		
		println("Started interpretation of \"" + stateMachine.getName() + "\".", interpreterOutputStream);
		...
		while (!currentState.isIsFinal()) {
			...
			for (Transition transition : transitions) {
				...
				if (transitionInput == null || transitionInput.equals("") || interpreterInput.startsWith(transitionInput)) {
			...
			if (numberOfEnabledTransitions == 0) {
				throw new QueryInterpreterException("No enabled transitions at state \"" + currentState.getName() + "\" for input \"" + interpreterInput + "\".");
			}
			
			//Check for non-determinism
			if (numberOfEnabledTransitions > 1) {
				throw new QueryInterpreterException("Non deterministics choice at state \"" + currentState.getName() + "\" for input \"" + interpreterInput + "\".");
			}
		...
		println("Finished interpretation in state \"" + currentState.getName() + "\".", interpreterOutputStream);
		println(interpreterOutputStream);
...
//	}
//	
//	private static Transition createTransition(State source, State target, String input, String output) {
