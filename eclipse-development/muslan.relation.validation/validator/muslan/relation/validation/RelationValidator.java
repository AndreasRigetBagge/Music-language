...
public class RelationValidator extends EObjectValidator implements IStartup {
	...
	private boolean validateRelationRoot(RelationRoot r) {
		// only a single main musiccollection
		long mcCount = r.getProviders().stream().filter(c -> c instanceof MainCollection).count();
		if (mcCount == 0) {
			return constraintViolated(r, "There must exist at least one main musiccollection");
		} else if (mcCount > 1) {
			var mcs = r.getProviders()
					.stream()
					.filter(c -> c instanceof MainCollection)
					.toArray(EObject[]::new);
			return constraintsViolated(mcs, "You may have at most 1 main musiccollection");
		}
		...
		// all collections must flow to (be reachable from) main musiccollection
		...
		if (reachables.size() != r.getProviders().size()) {
			...
			var msg = "All collections must indirectly"
					+ "  or directly flow to the main musiccollection";
	...
	// https://www.geeksforgeeks.org/detect-cycle-in-a-graph/#expected-approach-1-using-dfs-ov-e-time-and-ove-space
	...
	public boolean validateLink(Link link) {
		...
		if (link.getIncoming() instanceof MainCollection mc)
			return constraintViolated(link, String.format(
					"\"%s\" is not a valid input musiccollection - it is a main musiccollection", mc.getName()));
